"""Agent Beta (ELP) — A2A server on port 9402 with ToM + signaling.

Imports intelligence (LLM-enhanced ToM) and signaling from the l9 package.
Communicates with Agent Alpha using the ELP A2A message structure — every message
carries a structured L9 envelope with lexicons, grounding, belief, and history.

Run agent_alpha.py first, then this agent, then trigger.py.
"""
...
HOST, PORT = "localhost", 9402
PEER_ID = "alpha"
PEER_URL = "http://localhost:9401"
MAX_ROUNDS = 30
...
class BetaExecutor(AgentExecutor):
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        if nxt.get("decision") == "converged":
            out = build_l9(sender="beta", recipients=["alpha"],
                           episode=episode, data=nxt, parents=parents)
            print(f"[beta] {_label(nxt)}")
...
def main() -> None:
    ...
    print(f"Beta Agent (ELP + ToM) on http://{HOST}:{PORT}  (ext: {EXT_URI})")
