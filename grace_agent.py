"""Grace — stateless A2A ping-pong agent with Theory of Mind.

No state is stored in memory. All game state (lexicons, round, history,
beliefs about Rocky) travels in A2A message metadata.

Theory of Mind: Grace models what Rocky believes and predicts whether
he'll accept a proposal before sending it. She picks the proposal most
likely to succeed on the first try.

History: past proposals and outcomes travel in metadata so Grace never
...
from signaling import (
    MEANINGS, SYMBOLS, alignment, adopt,
...
class GraceExecutor(AgentExecutor):
    ...
    Each call:
      ...
      2. If Rocky just proposed: decide accept/reject using ToM
      3. If it's Grace's turn to propose: pick best proposal using ToM
      4. Send to Rocky or stop if converged
    ...
    async def execute(self, context: RequestContext, event_queue: EventQueue) -> None:
        ...
        if ctx is None:
            # Trigger from Mission Control — initialize conflicting lexicons
            ...
            # Rocky just proposed — decide whether to accept
            signal = md.get(MESSAGE)
            referent = md.get(REFERENT)
            if signal and referent:
                accepted = decide_accept(grace_lex, referent, signal, rocky_lex, history)
                if accepted:
                    adopt(grace_lex, referent, signal)
                history = record_outcome(history, referent, signal, accepted, "rocky")
        ...
        # Grace's turn — use Theory of Mind to pick the best proposal
        proposal = propose_with_tom(grace_lex, rocky_lex, history)
        if proposal is None:
            # Nothing to propose — shouldn't happen if not aligned, but safety net
            score = alignment(grace_lex, rocky_lex)
            summary = f"done | rounds: {rnd} | alignment: {score:.0%} | grace: {grace_lex} | rocky: {rocky_lex}"
            reply = Message(
        ...
        # Send to Rocky via A2A — full state in metadata
...
def build_agent_card(host: str = "localhost", port: int = 9101) -> AgentCard:
    ...
    ext = AgentExtension(uri=EXT, description="Game state + ToM via extension.", required=False)
    ext.params.update({"keys": ["context", "message", "referent", "response"], "max_rounds": MAX_ROUNDS})
    return AgentCard(
        name="Grace",
        description="Stateless ping-pong agent with Theory of Mind (the astronaut).",
        version="4.0.0",
        ...
        skills=[AgentSkill(
            ...
            description="Stateless ping-pong with ToM — predicts acceptance before proposing.", tags=["emergent"],
        )],
    )
